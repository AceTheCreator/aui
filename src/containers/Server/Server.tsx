...
export default function Server({
        ...
        Array.from(variables.keys()).map((variable, i) => {
          ...
          return (
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
              ...
              <dd className="mt-1 text-sm text-neutral-900 sm:mt-0 sm:col-span-2 prose">
                {variableProps.description}{" "}
                {variableProps.enum &&
                  formatEnumDescription(variableProps.enum)}
                <div className="mt-2">
                  <span className="font-bold text-neutral-500 mt-4 mr-2">
                    Default value:
                  </span>
  ...
  return (
    <div>
      <div className="font-bold text-neutral-700 mt-8 mb-4 bg-neutral-200 border border-neutral-500 p-4 rounded-lg">
        <div className="border border-dotted border-black p-2 rounded-lg bg-white">
          <IconLink className="inline-block mr-2 -mt-1 h-6 text-neutral-500" />
          {urlChunks}
        </div>
        <div className="mt-2">
          {tags &&
      ...
      {variables && (
        <>
          <h3 className="font-bold text-neutral-700 mt-8">
            <IconVariable className="inline-block mr-2 -mt-1 h-6 text-neutral-500" />
            URL Variables
          </h3>
          <div className="mt-5 border-t border-neutral-200">
            <dl className="sm:divide-y sm:divide-neutral-200">{variableElems}</dl>
          </div>
        </>
      )}
      {security && security.length > 0 && (
        <div>
          <h3 className="font-bold text-neutral-700 mt-8">
            <IconShieldCheck className="inline-block mr-2 -mt-1 h-6 text-neutral-500" />
            Authorization
          </h3>
          <p className="prose text-neutral-500 mt-4">
      ...
      {(() => {
        ...
        return (
          <div>
            <h3 className="font-bold text-neutral-700 mt-8">
              <Connection className="inline-block mr-2 -mt-1 h-6 text-neutral-500" />
              Connection Settings
            </h3>
            <p className="prose text-neutral-500 mt-4">
